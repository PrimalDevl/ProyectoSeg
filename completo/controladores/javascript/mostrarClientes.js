              ...
              <tr class="listaClientes">
                <td>
                      <div class="contenido vistaLista">
                          <p class="informacionCliente">${entrega.nombre} | ${entrega.apellido} | ${entrega.cedula} | ${entrega.comentario}
                          <button id="${entrega.cedula}" class="botonLista" type="button" name="button">Seleccionar</button>
                          </p>
                      </div>
                </td>
              </tr>
...
//Selecciona un elemento de la tabla y lo muestra en vista detallada
  $(document).on('click', '.botonLista', function() {
    traerListaClientes();
    let cedula = $(this).prop('id');
    document.cookie = "cedula = " + cedula + "; SameSite=Strict";
    ...
    $.ajax({
      ...
      success: function(datos) {
        ...
        cliente.forEach(entrega => {
          envoltorio += `
          <h1> ${entrega.nombre} ${entrega.apellido} </h1>
          <hr>
          <h2> ${entrega.cedula} </h2>
          <hr>
...
//Definicion de modificar datos
  $(document).on('click', "button[name=Modificar]", function() {
    ...
    <form class="" action="index.php" method="post">
      <input type="number" name="cedula" placeholder="cedula de Cliente">
